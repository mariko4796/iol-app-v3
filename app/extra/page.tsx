...
import {
  decideNext,
  type ExtraMode,
} from "@/src/domain/lens/scoring/decideNext";
import {
  pageInner,
  questionTitle,
  mainButtonStyle,
...
function ExtraPageInner() {
  ...
  const [premiumChoice, setPremiumChoice] =
    useState<"EDOF" | "MF" | null>(null);
  const [showScrollHint, setShowScrollHint] = useState(false);

  // 追加質問ページ用の音声オブジェクト
  const extraAudioRef = useRef<HTMLAudioElement | null>(null);

  // mode が変わったら選択状態をリセット
  useEffect(() => {
  ...
  const navigateNext = useCallback(
    (choice: {
      ...
      setTimeout(() => {
        ...
        if (mode === "retina") {
          if (choice.override) {
            const hasHaloNightRisk =
              flags.haloAbsoluteNo || flags.nightDrivingOften;
            if (hasHaloNightRisk) {
              router.push("/extra?mode=haloNight&afterRetina=1");
            } else {
              router.push(
                "/extra?mode=premiumCompare&afterRetina=1&overridePremium=1",
              );
            }
      ...
      {showScrollHint && (
        <div
          style={{
            ...
            maxWidth: 500,
            background:
              "radial-gradient(ellipse at center, rgba(15, 30, 78, 0.95) 0%, rgba(15, 30, 78, 0.7) 60%, rgba(15, 30, 78, 0) 100%)",
            color: "white",
            padding: "24px 50px",
            borderRadius: 40,
            fontSize: 28,
      ...
      <section style={pageInner}>
        {mode === "singleFocusChoice" && (
          <>
            ...
            <p style={extraDescriptionText}>
              問診票で<strong>「保険適用内のレンズを優先したい」</strong>
              と回答されたため、
              <strong>
                自費の多焦点レンズや EDOF レンズは今回の候補から外れます。
              </strong>
            </p>
            <p style={{ ...extraDescriptionText, marginTop: 8 }}>
              単焦点レンズは、
              <strong>遠方・中間・近方のどれか１つにピントを合わせるレンズ</strong>
              です。
            </p>
            <p style={{ ...extraDescriptionText, marginTop: 8 }}>
              今回のご回答では、遠方・中間・近方のすべてについて
              <strong>「裸眼で見たい」</strong>
              というご希望が強く出ています。しかし単焦点レンズでは、このすべてを同時に満たすことはできません。
            </p>
            <p style={{ ...extraDescriptionText, marginTop: 8 }}>
              そのため、
              <strong>最も大切な距離をもう一度お選びください。</strong>
            </p>
        ...
        {mode === "retina" && (
          <>
            <h2 style={questionTitle}>
              【網膜・緑内障などの眼疾患がある方へのご説明（重要）】
            </h2>
            <p>
              あなたのご回答（Q10）から、
              <strong>
                網膜・黄斑・緑内障などの眼の病気を指摘されたご経験
              </strong>
              があることが分かりました。
            </p>
            <p style={{ marginTop: 8 }}>
              こうした病気がある場合、
              <strong>
                多焦点レンズや EDOF レンズはコントラスト感度の低下や見え方の質の低下が強く出る可能性
              </strong>
              があるため、
              <strong>一般的には単焦点レンズを第一選択とします。</strong>
            </p>
            <p style={{ marginTop: 8 }}>
              また、
              <strong>
                白内障手術では水晶体の濁りは改善しますが、網膜や視神経そのものの病気が良くなるわけではありません。
              </strong>
            </p>
            <p style={{ marginTop: 8 }}>
              そのうえで、
              <strong>
                「どの距離を優先した単焦点レンズにするか」または「リスクを理解したうえで多焦点/EDOFを希望するか」
              </strong>
              をここで確認させてください。
            </p>
            <div style={{ marginTop: 16 }}>
              {distanceOptions.map((dist) => (
                <button
                  ...
                  style={mainButtonStyle(
                    distanceChoice === dist && !premiumOverride,
                  )}
                  onClick={() => handleDistanceSelect(dist)}
                >
                  {extraDistanceText[dist]}
              ...
              <button
                style={mainButtonStyle(premiumOverride)}
                onClick={handlePremiumOverride}
              >
                網膜や視神経の病気によるリスクを理解したうえで、多焦点レンズまたは
                EDOF レンズを希望する
              </button>
            </div>
          </>
        ...
        {mode === "haloNight" && (
          <>
            <h2 style={questionTitle}>
              【夜間運転とハロー・グレアに関するご説明（重要）】
            </h2>
            <p>
              あなたのご回答から、
              <strong>夜間に車の運転をよくされる（Q9：よくある）</strong>
              もしくは<strong>ハロー・グレアは「絶対にイヤ」（Q5：1）</strong>
              という強いご希望があることが分かりました。
            </p>
            <p style={{ marginTop: 8 }}>
              多焦点レンズや EDOF レンズでは、
              <strong>
                夜間のライトがにじんだり、ギラつき（ハロー・グレア）が出やすくなる
              </strong>
              ことがあります。特に夜間運転が多い方では、
              <strong>運転時の安全性に影響する可能性</strong>
              があるため、一般的には
              <strong>単焦点レンズを第一選択とします。</strong>
            </p>
            <p style={{ marginTop: 8 }}>
              そのうえで、
              <strong>
                「どの距離を優先した単焦点レンズにするか」または「夜間の見え方のリスクを理解したうえで多焦点/EDOFを希望するか」
              </strong>
              をここで確認させてください。
            </p>
            <div style={{ marginTop: 16 }}>
              ...
              <button
                ...
                onClick={handlePremiumOverride}
              >
                夜間の見え方（ハロー・グレアなど）のリスクを理解したうえで、多焦点レンズまたは
                EDOF レンズを希望する
              </button>
            </div>
          </>
        ...
        {mode === "premiumCompare" && (
          <>
            <h2 style={questionTitle}>
              【EDOF レンズと多焦点レンズのどちらを優先しますか？】
            </h2>
            <p>
              あなたのご回答から、
              <strong>
                EDOF レンズと多焦点レンズの両方が候補となり、スコアの差がほとんどない
              </strong>
              状態であると判定されました。
            </p>
            <p style={{ marginTop: 8 }}>
              どちらも「メガネを減らしたい」というご希望に沿うレンズですが、
              <strong>特に得意なポイントが少し違います。</strong>
            </p>
            <ul
              style={{
                marginTop: 8,
            ...
            >
              <li>
                <strong>EDOF レンズ：</strong>
                遠方〜中間の見え方をバランスよく保ちつつ、夜間のギラつき（ハロー・グレア）は多焦点レンズより少なめになることが多いレンズです。近く（スマホ・読書）ではメガネが必要になる場合があります。
              </li>
              <li style={{ marginTop: 6 }}>
                <strong>多焦点レンズ：</strong>
                遠方、中間、近方、
                <strong>できるだけ多くの距離を裸眼で見たい</strong>
                というご希望に合いやすい一方、夜間のギラつきやコントラスト低下が
                EDOF より強く出ることがあります。
              </li>
            </ul>
            <p style={{ marginTop: 8 }}>
              どの点をより優先したいかに応じて、
              <strong>
                どちらのレンズを優先するかをここでお選びください。
              </strong>
            </p>
            <div style={{ marginTop: 16 }}>
              <button
                style={mainButtonStyle(premiumChoice === "EDOF")}
                onClick={() => handlePremiumChoice("EDOF")}
              >
                EDOF レンズを優先したい（夜間のギラつきはできるだけ少なく、遠方〜中間をバランスよく見たい）
              </button>
              <button
                style={mainButtonStyle(premiumChoice === "MF")}
                onClick={() => handlePremiumChoice("MF")}
              >
